(function () {
  console.log("[Date Revealer] Running content script...");

  const scriptTag = document.querySelector(
    'script[type="application/ld+json"]'
  );
  if (!scriptTag)
    return console.warn("[Date Revealer] No ld+json script tag found.");

  const jsonData = JSON.parse(scriptTag.textContent);
  const datePosted = jsonData.datePosted;
  if (!datePosted)
    return console.warn("[Date Revealer] datePosted not found in JSON.");
  ...
  function getBadgeStyle({ minutes, hours }, posted) {
    ...
    if (minutes < 10) {
      borderColor = pillColor = "#DC2626";
      backgroundColor = "#FCF1F1";
      pillLabel = "🔥 Just posted";
    } else if (hours < 1) {
      borderColor = pillColor = "#d95c0f";
  ...
  function insertBadgeWhenReady(badge) {
    const interval = setInterval(() => {
      ...
      if (titleEl && !document.body.contains(badge)) {
        ...
        console.log("[Date Revealer] Badge (re)inserted.");
      }
    }, 500);

    setTimeout(() => {
      clearInterval(interval);
      console.log("[Date Revealer] Stopped checking after 10s.");
    }, 10000);
  }
})();
